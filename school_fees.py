...
class student_fees_structure(osv.osv):
    ...
    _columns = {
        'name':fields.char('Name', size=256, required=True),
        'code':fields.char('Code', size=64, required=True),
        'line_ids':fields.many2many('student.payslip.line', 'fees_structure_payslip_rel', 'fees_id', 'slip_id', 'Fees Stucture'),
...
class student_payslip(osv.osv):
            ...
            for datas in student_fees_structure_obj.browse(cr, uid, student_fees_structure_search_ids, context=context):
                for data in datas.line_ids:
                    student_payslip_line_vals = {
                            'slip_id':student_payslip_datas['id'],
                            'name':data.name,
                            'code':data.code,
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        """Creates studnet related financial move lines"""
        ...
        for fees in self.browse(cr, uid, ids, context=context):
            ...
            move_line = {
                ...
                'journal_id': fees.journal_id.id,
                'partner_id': fees.student_id.partner_id.id,
